...
### Type Assertions
***Interfaces*** in Go can introduce ambiguity about the underlying type. A type assertion 
allows us to extract the interface value's underlying concrete value using this syntax: 
`interfaceVariable.(concreteType)`. This is analogous to `isinstance(variable)` in python. <br>
...
> [!NOTE]
> This will cause a panic if the interface variable does not hold a value of the concrete type.
...
### Type Switches
A type switch (switch statement with multiple cases asserting variable types) can perform several type assertions in series. It has the same syntax as a type assertion `(interfaceVariable.(concreteType))`, but the concreteType is replaced with the keyword type. Here is an example:
```go
