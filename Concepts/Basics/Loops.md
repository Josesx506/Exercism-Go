### General syntax
The `for` loop is one of the most commonly used statements to repeatedly execute some 
logic. In Go it consists of the `for` keyword, a header and a code block that contains the 
body of the loop wrapped in curly brackets. The header consists of 3 components separated 
by semicolons `;`: *init*, *condition* and *post*.
```go
...
### While loops
Go does not have explicit while loops like other programming languages and uses for loops 
to check conditions. e.g.
```go
...
```
The `<` condition allows the loop to run till the condition is met like a while loop.